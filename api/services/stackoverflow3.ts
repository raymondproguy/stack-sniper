...
function extractKeySolution(answerBody, maxLines = 7) {
  ...
  for (let i = 0; i < Math.min(lines.length, maxLines + 3); i++) {
    ...
    if (line.includes('```') || line.includes('`') || 
        ...
        /[a-zA-Z]{10,}/.test(line)) { // Contains substantial text
  ...
  if (keyLines.length === 0) {
    keyLines = lines.slice(0, maxLines).filter(line => line.trim().length > 0);
  }
...
export async function searchStackOverflow(error) {
  try {
    ...
    const relevantItems = items.filter(item => {
      ...
      const hasRelevantTags = item.tags && item.tags.some(tag => 
        ['javascript', 'python', 'node.js', 'typescript'].includes(tag)
      );
    ...
    const question = relevantItems.sort((a, b) => b.score - a.score)[0];
